import {
  ...
  export const useUnfollowUser = () => {
    ...
    return useMutation({
      ...
      const documentToDelete = followers?.documents.find(
        (doc: any) => doc.followerId === followerId
      );
      ...
      // Use the document ID to unfollow
      return unfollowUser(documentToDelete.$id);
    },

      onSuccess: (_, { followerId, followingId }) => {
        queryClient.invalidateQueries({
          queryKey: [QUERY_KEYS.GET_FOLLOWERS,followingId], // Ensure GET_FOLLOWERS queries are refreshed
        });
        queryClient.invalidateQueries({
          queryKey: [QUERY_KEYS.GET_FOLLOWING,followerId], // Ensure GET_FOLLOWING queries are refreshed
        });
        queryClient.invalidateQueries({
          queryKey: [QUERY_KEYS.GET_CURRENT_USER], // Refresh user data
        });
      },
    });
  };
